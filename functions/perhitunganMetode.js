        ...
        if (rule) {
            ...
            if (matchedGejala.length > 0) {
                ...
                matchedGejala.forEach(gejala => {
                    ...
                    const cfUserBobot = parseFloat(localStorage.getItem(`CF_${gejala.kode}`)) || 0;
                    const cfResult = cfUserBobot * cfPakar;
...
document.getElementById('submitJawabanBtn').addEventListener('click', () => {
    ...
    if (result && result.disease) {
        const cfPercentage = (result.cf * 100).toFixed(0) + '%';
        const diagnosis = `Anda memiliki diagnosis awal ${result.disease.nama === 'Sindrom Dispepsia' ? '' : 'Penyakit '}${result.disease.nama} berdasarkan hasil perhitungan dengan nilai tertinggi yaitu ${cfPercentage}.`;
        const treatment = result.disease.treatment;
        const prevention = result.disease.prevention;
